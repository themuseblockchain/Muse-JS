...
Types.asset = {
    fromByteBuffer: function fromByteBuffer(b) {
        ...
        b.skip(7);
        // "1.000 MUSE" always written with full precision
    ...
    appendByteBuffer: function appendByteBuffer(b, object) {
        ...
        if (/^[0-9]+\.[0-9]+\.[0-9]+$/.test(symbol)) {
            var valInt = Math.trunc(v.to_long(amount.replace(".", "")) * Math.pow(10, 6 - precision));
            b.writeInt64(valInt);

            var symbolLastNumber = symbol.substring(symbol.lastIndexOf('.') + 1);
            var valPacking = Number(symbolLastNumber);
            do {
                var tmpByte = valPacking & 0x7f; // retrieve least significant 7 bits
            ...
            if (typeof value === "number") type_id = value;else {
                ...
                try {
                    for (var _iterator = this.st_operations[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        ...
                        if (op != null && op.operation_name === value) {
                            type_id = pos;
                            break;
                        }
                        pos++;
...
Types.obj_id_lastpart = {
    fromByteBuffer: function fromByteBuffer(b) {
        throw new Error("Not Implemented");
    },
    appendByteBuffer: function appendByteBuffer(b, object) {
        var parts = object.split(".");
        var valPacking = Number(parts[parts.length - 1]);
        do {
            var tmpByte = valPacking & 0x7f; // retrieve least significant 7 bits
            valPacking >>= 7; // Discard those bytes
            tmpByte |= valPacking > 0 == 1 ? 0x80 : 0; // If remaining bytes are greater than 0, it means there is still content after this byte
