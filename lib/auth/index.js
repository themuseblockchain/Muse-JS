...
/**
	...
	@arg {array} roles - defaults to standard Muse blockchain-level roles
*/
...
Auth.generateBalanceKeys = function (import_keys) {
	var results = [];
	import_keys.forEach(function (aKeyWif) {
		var pubKey = KeyPrivate.fromWifTruncate(aKeyWif).toPublic();

		// First address is simple, we only use buffer of pub key, which is then hashed
		results.push(Address.fromBuffer(pubKey.toBuffer()).toString());

		// The 4 others addresses are public key using different parameters
		results.push(Address.fromPublic(pubKey, false, 56).toString());
		results.push(Address.fromPublic(pubKey, true, 56).toString());
		results.push(Address.fromPublic(pubKey, false, 0).toString());
		results.push(Address.fromPublic(pubKey, true, 0).toString());
	});
...
Auth.wifToPublicSha256 = function (privWif) {
	var pubKeyBase58 = KeyPrivate.fromWifTruncate(privWif).toPublic().toPublicKeyStringSHA256();
	return pubKeyBase58;
};

Auth.fromPrivateWifTruncate = function (privWif) {
	return KeyPrivate.fromWifTruncate(privWif);
};
